...
class VAN(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 depths=[3, 4, 6, 3], num_stages=4, flag=False, k_size=7, pretrained_cfg=None):
        super().__init__()
        if flag == False:
        ...
        for i in range(num_stages):
            ...
            block = nn.ModuleList([Block(
                dim=embed_dims[i], k_size=k_size, mlp_ratio=mlp_ratios[i], drop=drop_rate, drop_path=dpr[cur + j])
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            if i == self.num_stages - 1:
                x_feature_map = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            if i != self.num_stages - 1:
                x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        return x_feature_map, x.mean(dim=1)

    def forward(self, x):
        _, x = self.forward_features(x)
        x = self.head(x)
